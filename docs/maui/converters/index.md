...
| [`ItemTappedEventArgsConverter`](item-tapped-eventargs-converter.md) | The `ItemTappedEventArgsConverter` is a converter that allows users to extract the Item value from an `ItemTappedEventArgs` object. It can subsequently be used in combination with EventToCommandBehavior. |
