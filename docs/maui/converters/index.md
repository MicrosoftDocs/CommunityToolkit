...
| [`EnumToBoolConverter`](enum-to-bool-converter.md) | The `EnumToBoolConverter` is a converter that allows you to convert a standard `Enum` to corresponding `bool` type. It is useful when binding a collection of values representing an enumeration type to a boolean control property like `IsVisible`. |
